...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($errors)) { 
		...
		$q = "SELECT ques_id FROM timu WHERE question='$ques' ";
		$r = mysql_query($q);

		if (mysql_num_rows($r) == 0) {
			// $q = "SELECT ques_id FROM timu ";
			// $r = mysql_query($q);
			// $line=mysql_affected_rows($link);
			// $line=$line+2;
			$line = $_POST['ques_id'];
			$q = "INSERT INTO timu (ques_id,question,answer,correct) VALUES ('$line','$ques','$an','$c')";
			$r = mysql_query ($q);
...
<p>题目：<input type="text" name="question" size="150" maxlength="150"  /></p>
<p>选项：<input type="text" name="answer" size="100" maxlength="100"  /></p>
<p>正确答案：<input type="text" name="correct" size="10" maxlength="10"   /> </p>
<p>题号：<input type="text" name="ques_id" size="10" maxlength="10"   /> </p>
